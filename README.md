...
Google's `VideoView` is great, and powerful, but I was not a fan at all of working with it. It was difficult, complex, and confusing. It also came with some key issues:

 - Bad aspect ratio and video stretching in some cases
 - Always took control of the `AudioManager` service (on Lollipop+)

This library looks to simplify the `VideoView`. It is simply a wrapper of Android's `MediaPlayer` that addresses some of the issues. It is also pretty light-weight up until you start the actual playback, so as long as you are smart about starting the videos, you could easily add this to a `RecyclerView` and use it for in-line video playback.
...
Note: This library does NOT contain any of the video controls (play, pause, fast-forward, rewind), although it does expose the methods necessary for you to implement them on your own. This may be something I add in the future.
